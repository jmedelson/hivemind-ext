...
const updateUser = async (word, question, user) =>{
       const params = {
            TableName: 'hivemind-user-data',
            Key: {
                "user": user,
            },
        ...
        let ret
        try{
            ret = await documentClient.update(params).promise();
        }catch(e){
            ret = "fail"
        }
        return(ret)
}
const updateStatic = async (word, question, channel) =>{
    const params = {
        TableName: 'hivemind-static-data',
        Key: {
            "question": question,
            "answer":word
        },
...
const updateDB2 = async (word, question, channel) =>{
    console.log("Updatedb2: "+ word, 'from-', channel);
    let params2 = {
    ...
    if(mapped){
        ...
        params = {
            ...
            UpdateExpression: 'ADD #a :y, #b :y, #c :y',
            ExpressionAttributeNames: {
                '#a' : 'count',
                "#b" : merged,
                "#c" : channels
    ...
    let catagory2 = "totals//"+question;
    const params4 = {
        TableName: 'hivemind-data',
        Key: {
            "placeholder": catagory2,
            "word": channel
        },
        UpdateExpression: 'ADD #a :y',
        ExpressionAttributeNames: {'#a' : 'count'},
    ...
    let [updated, updated2, updated3] = await Promise.all([documentClient.update(params).promise(), documentClient.update(params3).promise(),documentClient.update(params4).promise()])
    return updated
}
...
const voteDB = async (data,channel) =>{
...
const logStreamer = async (question, channel) =>{
...
const logTimeScene = async (lookup) =>{
    var params2 = {
        TableName: 'hivemind-ext',
        Key:{
            "data":"data"
    ...
    let data = await documentClient.get(params2).promise()
    console.log("logTimeScene", data)
    data = data['Item']
    if(lookup){
        var question = data['question']
    }
    else{
        var question = data['answer']
    }
    let time = new Date().toGMTString() 
        let params = {
            TableName: 'hivemind-timestamp',
            Item:{
                "time":time,
                "question":question
            }
        }
    let created = await documentClient.put(params).promise()
}
const logTimeQuestion = async (question) =>{
    var params2 = {
        TableName: 'hivemind-ext',
        Key:{
            "data":"data"
...
const logUser = async (user, userAgent, channel) =>{
    if(userAgent.length < 4){
        userAgent = "ERROR"
    }
    let params = {
        TableName: 'hivemind-users',
        Key:{
            "id": user,
        },
        UpdateExpression: 'ADD #a :y SET #b = :z, #c = :x',
        ExpressionAttributeNames: { 
            '#a': "count",
            '#b': "userAgent",
            '#c': "channel"
...
const logIp = async (ip,user) =>{
    let params = {
        TableName: 'hivemind-block-ip',
        Key:{
            "ip": ip,
        },
        UpdateExpression: 'SET #a = :y',
    ...
    if(parsed != null){
        ...
        if(parsed['flag']=="SET"){
            ...
            if(catagory=='question'){
                await logTimeQuestion(payload)
            }else if(catagory=='scene'){
                if(payload == 'poll'){
                    await logTimeScene(true)
                }else if(payload == 'agree'){
                    await logTimeScene(false)
                }
            }
            let ret = {
                id:catagory,
            ...
            // let [updated, check] = await Promise.all([updateDB2(payload,question,channel)]);
            let [updated, staticdata, userdata, check] = await Promise.all([updateDB2(payload,question,channel),updateStatic(payload,question,channel),updateUser(payload,question,viewer),updateCount(question),logStreamer(question,channel)]);
...
exports.handler = async (event) => {
    ...
    try {
        ...
        console.error("error occured in main")
        console.log("EVENT", event)
        console.log(e)
        console.error(e)
        var ret = {
